!> **IMPORTANTE**: Después de agregar este plugin a su `pubspec.yaml` deberá realizar la configuración en Android antes de ejecutar su proyecto nuevamente. Si no realiza la configuración del plugin obtendrá un error de tipo **No implementation found** debido a que el SDK de facebook tratara de encontrar la configuración necesaria y al no encontrar la configuración bloqueara a los demás plugins de su proyecto. Si aún no necesita el plugin comentelo de su archivo `pubspec.yaml`.
